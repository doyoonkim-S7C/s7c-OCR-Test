...
def process_single_pdf(pdf_path, output_dir, pages_per_batch, max_workers):
    ...
    output_filename = os.path.join(output_dir, f"{pdf_name}_claude_sonnet_4.0.mmd")
...
def generate_performance_report(performance_log, output_dir):
    ...
    report_filename = f"performance_report_claude_sonnet_4.0_{current_datetime}.txt"
    report_path = os.path.join(output_dir, report_filename)

    with open(report_path, 'w', encoding='utf-8') as report_file:
        # 헤더 정보
        report_file.write("=" * 80 + "\n")
        report_file.write("Claude Sonnet 4.0 OCR 성능 보고서\n")
        report_file.write("=" * 80 + "\n")

        # 모델 정보
        report_file.write("모델 정보:\n")
        report_file.write("-" * 40 + "\n")
        report_file.write("모델: AWS Bedrock Claude Sonnet 4.0\n")
        report_file.write("모델 ID: global.anthropic.claude-sonnet-4-20250514-v1:0\n")
